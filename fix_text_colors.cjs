...
// I will use a regex to replace the entire PRESETS block
...
// We need to update `applySpecificTheme` to read `theme.textColors` and inject it.
...
const applySpecificThemeReplace = `function applySpecificTheme(encodedTheme) {
    ...
    if (theme.textColors) {
        ...
        // Also update the UI color pickers if they are open/visible, though they might be in the individual palette popups
        // But the CSS variables will be updated by renderPreview()
    }
...
js = js.replace(applySpecificThemeRegex, applySpecificThemeReplace.trim());

fs.writeFileSync('app.js', js, 'utf8');
console.log('Massive presets added and textColors hooked up!');
